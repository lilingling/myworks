  ...
  $(document).ready(function(){
    ...
    function showResult(){
      ...
      if(correctCount >= 12){
        ...
        console.log('答对题的个数'+correctCount);
  ...
  $('input[name="tel"]').focus(function() {
    var defaultvalue1 = document.getElementById("phone_num1").defaultValue;
    var defaultvalue2 = document.getElementById("phone_num2").defaultValue;
    if($(this).val() == defaultvalue1 || $(this).val() == defaultvalue2){
      $(this).val('');
    }
  });

  //领取红包并抽奖
  $('#butn_getLottery').on('click',function(){
    $('.lottery-box').addClass('active');
  });
  $('#getBonus').on('click', function(){
    // 存储手机号
    if(checkPhone('phone_num1')){
      console.log('phone_num1====='+phone);
      ...
      $.ajax({
        ...
        success:function(){
          function isWeiXin(){
            ...
            var iPhone  = ua.indexOf('iphone') > -1 || ua.indexOf('ipad') > -1;
            if(ua.match(/MicroMessenger/i) == 'micromessenger'){
                return true;
            }
            else{
              if(android){
                window.location.href = 'wxmovie://hongbao?url=http://b.wepiao.com/hongbao/index.html?pid=151229f585e6583e&channelid=9&chid=10&p=%p%';
              }
              else if(iPhone){
                window.location.href= 'wxmovie://hongbao?url=http://b.wepiao.com/hongbao/index.html?pid=151229f585e6583e&channelid=8&chid=200&p=%p%';
              }
              return false;
            }
          }
          if(isWeiXin()){
              window.location.href = ' http://b.wepiao.com/hongbao/index.html?pid=151229f585e6583e&channelid=3&chid=100';
          }
        },
        error:function(){
          // alert('存储失败！');
    ...
    localStorage.phone_number=phone_numbers;
  ...
  if(localStorage.phone_number!=undefined){
    var temp = localStorage.phone_number.split(",");
    phone_numbers = temp;
  }
  // console.log('localstorage' +localStorage.phone_number);
  $('#getLottery').on('click',function(){
    if(checkPhone('phone_num2')){
      //查询手机号
      $.ajax({
        method:'GET',
        url:'http://api.biz.wepiao.com/common/queryactivityuserinfo?activeid=20160317001',
        dataType:'jsonp',
        success:function(data){
          console.log(data);
          console.log(phone);
          for (var item in data.data) {
            if (data.data[item].content == phone) {
              tag = true;
            }
            else{
            }
          ...
          if(tag){
            var isExist = phone_numbers.in_array(phone);
            console.log(isExist);
            if(!isExist){
              phone_numbers.push(phone);
              localStorage.phone_number=phone_numbers;
              $('#prompt1').show().text('恭喜你成功参与抽奖，如果中奖小票儿会与你电话联系，请注意接听哦.我们会在活动结束后5个工作日内于微票儿app公布获奖名单');

            }else{
              $('#prompt1').show().text('已经参与过了抽奖了');
            }
              console.log('locastorage ============'+localStorage.phone_number);
            tag = false;
          }
          else{
            $('#prompt1').show().text('你还没有获得抽奖资格，请先参与答题再来参加吧');
          }
        },
        error:function(){
          console.log('失败');
